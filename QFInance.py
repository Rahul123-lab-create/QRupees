...
c.execute("INSERT OR IGNORE INTO users (email, password, is_admin) VALUES (?, ?, 1)", (admin_email, admin_pass))
conn.commit()
...
if page == "Login" and not st.session_state.authenticated:
    ...
    if st.button("Login"):
        ...
        if user and check_password(user[1], password):
            c.execute("SELECT approved FROM registrations WHERE user_id=?", (user[0],))
            reg = c.fetchone()
            if reg and reg[0] == 1 or user[2] == 1:  # Approved or admin
        ...
        if submit_button:
            ...
            if not all([full_name, email, password, phone, address, city, country, highest_degree != "Select your degree", trading_duration != "Select duration", about_yourself, consent]):
                ...
                if c.fetchone():
                    ...
                    c.execute("""INSERT INTO registrations (user_id, full_name, phone, address, city, state, zip_code, country, 
                                 ...
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)""", 
                              (user_id, full_name, phone, address, city, state, zip_code, country, highest_degree, field_of_study, 
...
if st.session_state.authenticated:
    if page == "Dashboard":
        ...
        c.execute("SELECT r.id, u.email, r.full_name, r.approved FROM registrations r JOIN users u ON r.user_id = u.id WHERE r.approved = 0")
        pending = c.fetchall()
        for reg in pending:
            st.write(f"Email: {reg[1]}, Name: {reg[2]}")
            if st.button("Approve", key=reg[0]):
                c.execute("UPDATE registrations SET approved=1 WHERE id=?", (reg[0],))
                conn.commit()
                st.success("Approved!")
                st.rerun()
